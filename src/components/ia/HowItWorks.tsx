...
export default function HowItWorks() {
  ...
  const validSteps = useMemo(() => {
    ...
    return content.steps.filter(
      (step): step is Step =>
        step && typeof step === 'object' && 'title' in step && 'desc' in step
    );
  }, [content.steps]);
          ...
          {validSteps.map((step, index) => {
            ...
            return (
              <motion.article
                ...
                className="nexus-card-interactive relative rounded-2xl border border-border/50 bg-card/60 p-6 backdrop-blur-xl transition-all duration-300 focus-within:ring-2 focus-within:ring-neon-purple/50"
                role="region"
